...
BOOL CNuWriterDlg::OnInitDialog()
{
	...
	if(target.DoModal()==IDCANCEL) exit(0);	
	...
	CString tempText;
	if(!Auto_Detect(m_portName, tempText))
	{
		m_portName="";
		tempText=" Disconnected";
...
void CNuWriterDlg::OnBnClickedReconnect()
{
	...
	#if 1
	{
		...
		CString temp2=path2;	
		temp2 = temp2.Left(temp2.ReverseFind('\\') + 1);	
		CString filename;	
		filename.Format(_T("%sxusb.bin"),temp2);
		fp=_wfopen(filename,_T("rb"));
		if(fp)
		{		
			fread((char*)&xbinhead,sizeof(XBINHEAD),1,fp);				
			version.Format(_T(" 20%02x/%02x/%02x-V%02x"),(xbinhead.version>>24)&0xff,(xbinhead.version>>16)&0xff,(xbinhead.version>>8)&0xff,(xbinhead.version)&0xff);
...
void CNuWriterDlg::OnCbnSelchangeComboType()
{
	...
	m_type.SetWindowText(title);
	m_SubForms.ShowSubForm(m_gtype.GetCurSel());
	//encoder=((CComboBox*)GetDlgItem(IDC_COMBO_TYPE))->GetCurSel();
