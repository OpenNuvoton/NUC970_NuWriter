...
int Run_SDRAM(UINT32 address,UINT32 offset,UINT32 tmpAddr)
{
    unsigned int i;
...
{
    unsigned int actionFlag=0, fileAddr=0, fileSize=0, exeFlag=0;
    int len;
    unsigned char *ptr;
    unsigned char buf[80];
    unsigned char *_ch;
  ...
  unsigned int tmpAddr=((EXEADDR-0x100000)-offset);
    ...
    MSG_DEBUG("action flag %d, address 0x%x, len %d\n", actionFlag, fileAddr, fileSize);
  ...
  if(fileAddr<offset && exeFlag == 1) {
    ...
    if (exeFlag == 1)   /* execute image */
    {
        ...
        #endif
        Run_SDRAM(fileAddr,offset,tmpAddr);
    }
    else
    ...
    switch(pmmcImage->actionFlag)
    {
        case WRITER_MODE: /* normal write */
            ...
            if (pmmcImage->imageType == UBOOT)
            {
                ...
                #else
                BatchBurn_MMC(pmmcImage->fileLength+pmmcImage->initSize,(pmmcImage->flashOffset/SD_SECTOR),1);
                #endif
            }else{
                #if !defined(BATCH_BRUN)
                Burn_MMC(pmmcImage->fileLength,(pmmcImage->flashOffset/SD_SECTOR));
                #else
                BatchBurn_MMC(pmmcImage->fileLength,(pmmcImage->flashOffset/SD_SECTOR),0);
                #endif
            }
    ...
    case WRITER_MODE:   // normal write
    {
      ...
      if (pNandImage->imageType == UBOOT){    // system image
        ...
        if(pNandImage->imageType!=IMAGE)
        {
          ...
          {
          #if !defined(BATCH_BRUN)
          Burn_NAND(pNandImage->fileLength + offset +((FW_NAND_IMAGE_T *)pNandImage)->initSize,pNandImage->blockNo/(pSM->uPagePerBlock*pSM->uPageSize),pNandImage->imageType);Burn_NAND(pNandImage->fileLength + offset +((FW_NAND_IMAGE_T *)pNandImage)->initSize,pNandImage->blockNo/(pSM->uPagePerBlock*pSM->uPageSize),pNandImage->imageType);
          #else
              ...
              for(i=0;i<pack.num;i++)
              {
                  ...
                  if(ppack.imagetype!=PMTP)
                  {
                    if(ppack.imagetype==UBOOT)
                    {
                        ...
                        #else
                        BatchBurn_NAND_BOOT(ppack.filelen,0,4,0);
                        #endif
                    }else{
                      ...
                      else
                      {
                        ...
                        if(((char)_ch[0])=='U' && ((char)_ch[1])=='B' && ((char)_ch[2])=='I'){
                          ...
                          #else
                          BatchBurn_NAND(ppack.filelen,ppack.startaddr/(pSM->uPageSize*pSM->uPagePerBlock),UBIFS);
                          #endif
                        }else{
                          #if !defined(BATCH_BRUN)
